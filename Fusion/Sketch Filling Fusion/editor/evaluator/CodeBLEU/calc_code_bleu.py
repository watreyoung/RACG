...
def get_codebleu(refs, hyp, lang, params='0.25,0.25,0.25,0.25'):
    ...
    # preprocess inputs
    pre_references = [[json.loads(x)["target"].strip() for x in open(file, 'r', encoding='utf-8').readlines()] for file in refs]
    hypothesis = [json.loads(x)["prediction"].strip() for x in open(hyp, 'r', encoding='utf-8').readlines()]
