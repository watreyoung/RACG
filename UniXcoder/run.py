        ...
        for epoch in range(args.num_train_epochs):
            ...
            if args.do_eval:
                ...
                if 'dev_bleu' in dev_dataset:
                    ...
                    eval_examples = read_examples(args.dev_filename)
                    eval_examples = random.sample(eval_examples,max(60,len(eval_examples)))
                    # HS 66
                    # eval_examples = random.sample(eval_examples,max(1000,len(eval_examples)))
                ...
                if epoch >= 0:
                    ...
                    with open(args.output_dir+"/{}dev.output".format(epoch),'w') as f, open(args.output_dir+"/dev.gold",'w') as f1:
                #     i = 0
                #     j = 0
                #     while i < len(eval_examples):
                #         gold = eval_examples[i]
                #         f1.write(gold.target+'\n')
                #         ref = p[j]
                #         while j < len(p)-1 and (p[j+1].startswith("\t") or p[j+1].startswith("    ")):
                #             ref += p[j+1]
                #             j += 1
                #         f1.write(ref+'\n')
                ...
                #     codebleu = calc_code_bleu.get_codebleu(args.output_dir+"/dev.gold", args.output_dir+"/dev.output", "java")
                #     logger.info("  %s = %s"%("code-bleu",str(codebleu)))
                ...
                #         logger.info("  "+"*"*20)    
                #         logger.info("The most best model is {}".format(epoch))    
                ...
                #     print(e)
    if args.do_test:
        checkpoint_prefix = '9_pytorch_model.bin'
        ...
        predictions=[]
        EM = []
        with open(args.output_dir+"/test.gold","w")as f1, open(args.output_dir+"/predictions.txt",'w') as f:
            for ref,gold in zip(p,eval_examples):
                predictions.append(str(gold.idx)+'\t'+ref)
                f1.write(gold.target+'\n')
                f.write(ref+'\n') 
                EM.append(ref.split()==gold.target.split())  
        dev_bleu = _bleu(args.output_dir+"/test.gold", args.output_dir+"/predictions.txt") 
        codebleu = calc_code_bleu.get_codebleu(args.output_dir+"/test.gold", args.output_dir+"/predictions.txt", args.lang)
        logger.info("  %s = %s"%("code-bleu",str(codebleu)))
        logger.info("  %s = %s "%("bleu-4",str(dev_bleu)))
        logger.info("  %s = %s "%("EM",str(round(np.mean(EM)*100,2))))
        logger.info("  "+"*"*20)    
