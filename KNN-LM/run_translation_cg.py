...
class DataTrainingArguments:
    ...
    load: Optional[str] = field(
        default=None,
        metadata={"help": "An optional input one checkpoint for models."},
    )
...
def main():
    ...
    # yzz add
    if data_args.load is not None:
        model.load_state_dict(torch.load(data_args.load))
    ...
    if training_args.do_eval:
        ...
        print('$'*200)
        print(f'[{data_args.eval_subset}] Total eval tokens: {total_eval_tokens}')
    ...
    metric = evaluate.load("./metrics/bleu")
    ...
    def compute_metrics(eval_preds):
        ...
        with open('{}/preds.txt'.format(training_args.output_dir),'w') as preds_txt, open("{}/labels.txt".format(training_args.output_dir),'w') as labels_txt:
            # print(len(decoded_preds))
            # print(len(decoded_labels))
            # decoded_labels = [pp[0] for pp in decoded_labels]
            # print(decoded_preds)
            # print(decoded_labels)
            for pred in decoded_preds:
                preds_txt.write(pred.replace('\r',' ').replace("\n"," "))
                preds_txt.write("\n")
            for label in decoded_labels:
                for l in label:
                    labels_txt.write(l.replace('\r',' ').replace("\n"," "))
                    labels_txt.write("\n")
    ...
    #     metrics = trainer.evaluate(max_length=max_length, num_beams=num_beams, metric_key_prefix="test")
    #     max_eval_samples = data_args.max_eval_samples if data_args.max_eval_samples is not None else len(eval_dataset)
    #     metrics["eval_samples"] = min(max_eval_samples, len(eval_dataset))
