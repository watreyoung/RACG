...
def train(args, model, tokenizer,pool):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        output_dir = os.path.join(args.output_dir, '{}'.format(idx))
...
def evaluate(args, model, tokenizer,file_name,pool, eval_when_training=False):
    ...
    for example in query_dataset.examples:
        res_nl.append(" ".join(example.nl_tokens))
        temp_code.append(" ".join(example.code_tokens))
    print(res_nl[0])
    ...
    if args.save_evaluation_reuslt:
        evaluation_result = {"nl_tokens":res_nl, "code_urls":res_code,"sort_ids":sort_ids[:,:10],"ranks":ranks}
