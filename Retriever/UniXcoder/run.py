...
def train(args, model, tokenizer):
    ...
    for idx in range(args.num_train_epochs): 
        ...
        print('Epoch {} has done'.format(idx))
    logger.info("  "+"*"*20)  
...
def evaluate(args, model, tokenizer,file_name,eval_when_training=False):
    ...
    try:
        for example in query_dataset.examples:
            xxx = tokenizer.decode(tokenizer.encode(example.nl_tokens[3:-1])).replace('<s>','').replace('</s>','')
            res_nl.append(xxx)
            temp_code[xxx] = tokenizer.decode(tokenizer.encode(example.code_tokens[3:-1])).replace('<s>','').replace('</s>','')
        # print(res_nl[0])
            
        for example in code_dataset.examples:
            res_code.append(tokenizer.decode(tokenizer.encode(example.code_tokens[3:-1])).replace('<s>','').replace('</s>',''))
            temp_nl.append(tokenizer.decode(tokenizer.encode(example.nl_tokens[3:-1])).replace('<s>','').replace('</s>',''))
    except Exception as e:
        print(e)
        length = min(len(res_nl),len(temp_code),len(res_code),len(temp_nl))
        res_nl = res_nl[:length]
        res_code = res_code[:length]
        temp_code = temp_code[:length]
        temp_nl = temp_nl[:length]

    # print(res_code[0])
    # print(res_nl[0]==temp_nl[0])
    # print(res_code[0]==temp_code[0])
    if 'test' in file_name:
        retrieved_file = os.path.join(args.output_dir,'checkpoint-last/test.json')
    elif 'train' in file_name:
        retrieved_file = os.path.join(args.output_dir,'checkpoint-last/train.json')
    else:
        retrieved_file = os.path.join(args.output_dir,'checkpoint-last/dev.json')
...
def main():
    ...
    # Evaluation
    checkpoint_prefix = 'checkpoint-last/model.bin'
    output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))  
    ...
    if args.do_eval:
        ...
        evaluate(args, model, tokenizer, args.train_data_file)
    if args.do_test:
