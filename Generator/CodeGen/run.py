...
def train(args, train_dataset, model, tokenizer, fh, pool):
    ...
    if args.num_train_epochs > 0:
        ...
        # chensr
        # examples==100000;batch_size==12;train_epochs==30;计算结果为249990
    ...
    for idx in range(args.start_epoch, int(args.num_train_epochs)): 
        for step, (batch, token_labels) in enumerate(train_dataloader):
            ...
            attn_mask = torch.as_tensor(token_labels.clone().detach() != 0, dtype=torch.uint8, device=args.device)
            loss_mask = torch.as_tensor(token_labels.clone().detach() == 2, dtype=torch.uint8, device=args.device)
            model.train()
            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if args.evaluate_during_training:  # Only evaluate when single GPU otherwise metrics may not average well
                        ...
                        dev_bleu, dev_EM, codebleu= eval_bleu(args, model, tokenizer, file_type='dev', num=2000)
                        logger.info(f"dev EM: {dev_EM}, dev bleu: {dev_bleu},  dev codebleu: {codebleu}")
                        test_bleu, test_EM, test_codebleu = eval_bleu(args, model, tokenizer,file_type="test", num=2000)
                        logger.info(f"test EM: {test_EM}, test bleu: {test_bleu},  test codebleu: {test_codebleu}")
                        output_dir = os.path.join(args.output_dir, '{}-{}-{}'.format(checkpoint_prefix, global_step, round(test_bleu,2)))
                        # if dev_EM + dev_bleu + codebleu > best_EM + best_bleu + best_codebleu:
                        #     best_bleu = dev_bleu
                        #     best_EM = dev_EM
                        #     best_codebleu = codebleu
                        #     logger.info(f"best model updated. saved in {output_dir}")
                        #     logger.info(f"best EM:{best_EM}")
                        #     logger.info(f"best bleu: {best_bleu}")
                        #     logger.info(f"best codebleu: {best_codebleu}")
                    else:
...
def eval_bleu(args, model, tokenizer, file_type='test', num=2000):
    ...
    # datafile = os.path.join(args.data_dir, f"conala-{file_type}.json")
    # datafile = os.path.join(args.data_dir, f"{file_type}_bm25_cat2.json")
    # datafile = os.path.join(args.data_dir, f"hearthstone-{file_type}.json")
    ...
    with open(os.path.join(args.output_dir, f"{file_type}.output"), 'w') as f, open(os.path.join(args.output_dir, f"{file_type}.gold"), 'w') as f1:
        for pred, gold in zip(preds, golds):
            # chensr 防止输出\n
            # f.write(pred+'\n')
            f.write(pred.replace('\r',' ').replace('\n',' ')+'\n')
            f1.write(gold.replace('\r',' ').replace('\n',' ')+'\n')  
...
def main():
    ...
    # chensr torch.distributed.launch时local_rank为0，直接run为-1。
    if args.local_rank == -1 or args.no_cuda:
        logger.warning("branch1")
        device = torch.device("cuda" if torch.cuda.is_available() and not args.no_cuda else "cpu")
        args.n_gpu = torch.cuda.device_count()
    else:  # Initializes the distributed backend which will take care of sychronizing nodes/GPUs
        logger.warning("branch2")
        torch.cuda.set_device(args.local_rank)
        device = torch.device("cuda", args.local_rank)
        torch.distributed.init_process_group(backend='nccl')
        args.local_rank += args.node_index * args.gpu_per_node
        args.n_gpu = 1
    # args.n_gpu = 1  # chensr GPU数量默认设为1
    args.device = device
    # args.batch_size = args.per_gpu_train_batch_size * max(1, args.n_gpu) 
    ...
    tokenizer = AutoTokenizer.from_pretrained("./codegen-350M-mono", bos_token='<s>', eos_token='</s>', pad_token='<pad>', unk_token='<|UNKNOWN|>', sep_token='concode_elem_sep')
    args.vocab_size = tokenizer.vocab_size
    ...
    if args.do_eval:            # only works on 1 GPU
        ...
        dirs = [os.path.join(args.output_dir,f) for f in os.listdir(args.output_dir) if os.path.isdir(os.path.join(args.output_dir,f))]
        print(dirs)
        dirs.sort()
        best_EM, best_bleu ,best_codebleu = 0,0,0
        for dir in dirs:
            output_dir = os.path.join(dir, "pytorch_model.bin")  
            model = model.module if hasattr(model, 'module') else model  
            model.load_state_dict(torch.load(output_dir))  
            dev_bleu, dev_EM, dev_codebleu= eval_bleu(args, model, tokenizer, file_type='dev', num=2000)
            if dev_bleu + dev_EM + dev_codebleu > best_EM + best_bleu + best_codebleu:
                best_EM = dev_EM
                best_bleu = dev_bleu
                best_codebleu = dev_codebleu
                logger.info(f"best model has been updated :{dir}____dev bleu: {dev_bleu}, dev EM: {dev_EM}, dev codebleu: {dev_codebleu}")
            logger.info(f"{dir}____dev bleu: {dev_bleu}, dev EM: {dev_EM}, dev codebleu: {dev_codebleu}")

    if args.do_infer:            # only works on 1 GPU
        test_bleu, test_EM, test_codebleu= eval_bleu(args, model, tokenizer, file_type='test', num=2000)
        logger.info(f"test bleu: {test_bleu}, test EM: {test_EM}, test codebleu: {test_codebleu}")
