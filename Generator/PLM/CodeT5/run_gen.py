...
def eval_bleu_epoch(args, eval_data, eval_examples, model, tokenizer, split_tag, criteria):
    ...
    with open(output_fn, 'w') as f, open(gold_fn, 'w') as f1, open(src_fn, 'w') as f2:
        ...
        if args.task in ['concode', 'conala', 'hearthstone']:
...
def main():
    ...
    logger.info(args)      # INFO - __main__ -   Namespace
    t0 = time.time()
    set_dist(args)       
    set_seed(args)
    ...
    if args.do_train:
        ...
        for cur_epoch in range(args.start_epoch, int(args.num_train_epochs)):
            bar = tqdm(train_dataloader, total=len(train_dataloader), desc="Training", mininterval=5)
            ...
            for step, batch in enumerate(bar):
                ...
                if nb_tr_steps % args.gradient_accumulation_steps == 0:
                    ...
                    if step == 3:
                        bar.set_description(" Memory_allocated {}. Memory_reserved {} ".format(torch.cuda.memory_allocated(),torch.cuda.memory_reserved()))
